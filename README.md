...
## Quickstart - single file  webapp with self-installation

for quick demos, can create and share examples which can be run directy with `uv run`

For exmaple, helloworld.py
```python
# /// script
# dependencies = [
...
```
will run by creating a temporary environemnt, download dependencies and run the
script with:
...
## using the nanodjango interface
Alternatively, you can run the bolt server with nanodjango's CLI:

```bash
nanodjango manage myapp.py runbolt --port 8000
```
In this case, you don't need the "if __main__" part of the code
